...
[English](README.md) | [Italiano](README_IT.md) | [Français](README_FR.md) | [Deutsch](README_DE.md) | 中文简体 | [中文繁體](README_ZH_TW.md) | [日本語](README_JA.md) | [한국어](README_KO.md) | [Português](README_PT_BR.md) | [Русский](README_RU.md) | [Español](README_ES.md)

**Escape from Duckov** 的多功能生活品质模组。22个独立功能，均可从原生**设置**菜单中配置。
...
**目录:** [功能](#功能) · [拾取](#-拾取) · [战斗](#-战斗) · [生存](#-生存) · [HUD](#-hud) · [任务](#-任务) · [性能](#性能) · [安装](#安装) · [更新日志](#更新日志)
...
![战利品箱高亮](assets/lootbox-highlight-coloured.png)
...
#### 黑市基础价格
在每张黑市卡片上显示基础价格和百分比差异。对玩家有利的报价以绿色高亮显示，不利的以红色显示。可从设置中切换（默认开启）。

#### 按价值排序（排序按钮）
将库存排序按钮替换为4种模式循环：原生 / 价值 / 堆叠价值 / 每千克价值。可从设置中切换（默认开启）。
...
大多数功能由事件驱动，对性能没有可测量的影响。少数功能会定期扫描场景，可能在低端硬件上造成短暂卡顿。如果遇到帧率下降，请按影响顺序优先禁用以下功能：

| 功能               | 峰值/场次 | 平均峰值 | 总开销 | %   |
| ------------------ | --------- | -------- | ------ | --- |
| 显示隐藏的敌人血条 | 32        | ~24ms    | ~768ms | 34% |
| 物品稀有度显示     | 43        | ~13ms    | ~559ms | 25% |
| 战利品箱高亮       | 32        | ~14ms    | ~448ms | 20% |
| 显示敌人名称       | 32        | ~13ms    | ~416ms | 18% |
| Boss地图标记       | 5         | ~15ms    | ~75ms  | 3%  |
