...
**목차:** [기능](#기능) · [루팅](#-루팅) · [전투](#-전투) · [생존](#-생존) · [HUD](#-hud) · [퀘스트](#-퀘스트) · [성능](#성능) · [설치](#설치) · [변경 로그](#변경-로그)
...
![루트박스 강조 표시](assets/lootbox-highlight-coloured.png)
...
대부분의 기능은 이벤트 기반으로 작동하며 프레임 성능에 측정 가능한 영향이 없습니다. 일부 기능은 주기적으로 씬을 스캔하여 저사양 하드웨어에서 짧은 끊김을 유발할 수 있습니다. 프레임 저하가 발생하면 영향 순서대로 다음 기능을 먼저 비활성화해 보세요:

| 기능                   | 스파이크/세션 | 평균 스파이크 | 총 오버헤드 | %   |
| ---------------------- | ------------- | ------------- | ----------- | --- |
| 숨겨진 적 체력 바 표시 | 32            | ~24ms         | ~768ms      | 34% |
| 아이템 희귀도 표시     | 43            | ~13ms         | ~559ms      | 25% |
| 루트박스 강조 표시     | 32            | ~14ms         | ~448ms      | 20% |
| 적 이름 표시           | 32            | ~13ms         | ~416ms      | 18% |
| 보스 지도 마커         | 5             | ~15ms         | ~75ms       | 3%  |
