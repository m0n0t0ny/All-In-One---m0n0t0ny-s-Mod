...
**Escape from Duckov** 的多功能生活品質模組。22個獨立功能，均可從原生**設定**選單中配置。
...
#### 黑市基礎價格
在每張黑市卡片上顯示基礎價格和百分比差異。對玩家有利的報價以綠色高亮顯示，不利的以紅色顯示。可從設定中切換（預設開啟）。

#### 按價值排序（排序按鈕）
將庫存排序按鈕替換為4種模式循環：原生 / 價值 / 堆疊價值 / 每公斤價值。可從設定中切換（預設開啟）。
