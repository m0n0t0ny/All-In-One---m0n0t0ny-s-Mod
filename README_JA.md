...
**Escape from Duckov** のオールインワン・クオリティオブライフMod。22の独立した機能を搭載し、すべてネイティブの**設定**メニューから設定可能です。
...
#### ブラックマーケット基準価格
各ブラックマーケットカードに基準価格と割合の差異を表示します。プレイヤーに有利なオファーは緑、不利なものは赤でハイライトされます。設定からトグル可能（デフォルトでオン）。

#### 価値でソート（ソートボタン）
インベントリのソートボタンを4モードサイクルに置き換えます：ネイティブ / 価値 / スタック価値 / kg当たり価値。設定からトグル可能（デフォルトでオン）。
