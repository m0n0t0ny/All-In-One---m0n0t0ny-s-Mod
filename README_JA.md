...
**目次:** [機能](#機能) · [ルーティング](#-ルーティング) · [戦闘](#-戦闘) · [サバイバル](#-サバイバル) · [HUD](#-hud) · [クエスト](#-クエスト) · [パフォーマンス](#パフォーマンス) · [インストール](#インストール) · [変更履歴](#変更履歴)
...
![ルートボックスハイライト](assets/lootbox-highlight-coloured.png)
...
ほとんどの機能はイベント駆動型であり、フレームパフォーマンスへの影響は測定不可能です。一部の機能は定期的にシーンをスキャンするため、低スペックのハードウェアでは短いスタッターが発生することがあります。フレームレートが低下する場合は、影響の大きい順に以下の機能を無効にしてください：

| 機能 | スパイク/セッション | 平均スパイク | 合計オーバーヘッド | % |
|---|---|---|---|---|
| 隠れた敵のHPバーを表示 | 32 | ~24ms | ~768ms | 34% |
| アイテムレアリティ表示 | 43 | ~13ms | ~559ms | 25% |
| ルートボックスハイライト | 32 | ~14ms | ~448ms | 20% |
| 敵の名前を表示 | 32 | ~13ms | ~416ms | 18% |
| ボスマップマーカー | 5 | ~15ms | ~75ms | 3% |
